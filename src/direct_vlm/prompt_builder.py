"""构建 DirectVLM 模块的 System / User prompt.

将卷面识别和评分合并为一个 VLM 请求：模型直接读取手写图片并按评分标准打分。
"""

from __future__ import annotations

from src.model.types import InputAsset
from src.judge.types import ScoringItem, ScoringRubric


_SYSTEM_PROMPT = """\
Role: 你是一位经验丰富的物理竞赛阅卷专家，同时精通手写数学公式识别。

Task: 你将收到学生手写答卷的图片和评分标准。请你：
1. 直接阅读手写答卷图片，理解学生的作答内容。
2. 根据提供的评分标准，对学生作答进行逐项评分。
...
8. 仔细辨认手写内容，对于难以辨认的部分应尽量根据上下文推断。
9. 忽略划掉的内容。
10. 输出必须严格为 JSON 格式，不要添加任何额外文字。
"""


def build_direct_vlm_messages(
    assets: list[InputAsset],
    rubric: ScoringRubric,
    verbosity: int = 1,
) -> list[dict[str, object]]:
    """Construct the messages for a single VLM call that reads images and scores."""
    user_text = _build_user_prompt(rubric, verbosity)
...
def _build_user_prompt(rubric: ScoringRubric, verbosity: int) -> str:
    sections: list[str] = []
